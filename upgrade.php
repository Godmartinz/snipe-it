...
function url_get_contents($Url)
{
    ...
    curl_close($ch);
    if ($output === false) {
        echo "\e[91mError retrieving PHP requirements!\e[39m\n";
...
if (! $upgrade_requirements) {
    if (! $skip_php_checks) {
        ...
        if (json_last_error()) {
            echo "\e[91mJSON DECODE ERROR DETECTED:\n";
            echo json_last_error_msg()."\n\n";
            echo "Raw curl output:\n";
            echo $upgrade_requirements_raw."\e[39m\n\n";
        }
...
if ((strpos('git version', $git_version)) === false) {
    ...
    foreach (explode("\n", $remote) as $line) {
        ...
        @[$url, $purpose] = explode(' ', $remote_bits[1]);
...
echo '-- '.implode("\n", $down_results)."\n";
if ($return_code > 0) {
    exit("Something went wrong with downing your site. This can't be good. Please investigate the error and be sure to check https://snipe-it.readme.io/docs/common-issues and https://snipe-it.readme.io/docs/installation-issues for solutions to common upgrading issues. Aborting!\n\n");
}
unset($return_code);
...
if (! $skip_backup) {
    ...
    if ($return_code > 0) {
        exit($error_icon." Something went wrong with your backup. Aborting!\n\n");
    } else {
        echo '-- '.implode("\n", $backup_results)."\n\n";
    }
    unset($return_code);
